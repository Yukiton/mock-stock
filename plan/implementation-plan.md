...
| 阶段 | 任务 | 优先级 | 预计工作量 |
|------|------|--------|------------|
| **Phase 1** | 项目初始化、配置、数据库模型 | P0 | 基础框架搭建 |
| **Phase 2** | 用户API、持仓管理API | P0 | 核心CRUD |
| **Phase 3** | 交易API、资产估值API（集成akshare行情） | P0 | 核心交易逻辑 |
| **Phase 4** | 价格提醒系统（策略抽象+通知器抽象） | P1 | 扩展功能 |
| **Phase 5** | 定时任务系统（APScheduler集成） | P1 | 自动化 |
| **Phase 6** | WebSocket实时推送 | P2 | 实时性增强 |
| **Phase 7** | 单元测试与文档 | P2 | 质量保障 |
